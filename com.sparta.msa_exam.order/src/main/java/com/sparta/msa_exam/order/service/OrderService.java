...
public class OrderService {
    ...
    @Transactional
    public OrderResponse createOrder(OrderAddRequest request, String userId) {
        // 1. 상품 서비스에서 상품 목록 조회
        ...
        // 4. 주문 생성 및 저장
        Order order = orderRepository.save(request.toOrder(UUID.fromString(userId)));
    ...
    @Transactional
    public OrderResponse updateOrder(OrderUpdateRequest request, Long orderId, String userId) {
        // 1. 주문 유효성
    ...
    public OrderGetResponse getOrder(Long orderId, String userId) {
        Order order = orderRepository.findById(orderId)
                .orElseThrow(() -> new IllegalArgumentException("No valid order"));
